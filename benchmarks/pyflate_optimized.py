...
BIT_MASKS = tuple((1 << bits) - 1 for bits in range(65))
...
class Bitfield(BitfieldBase):
    ...
    def _more(self):
        ...
        self.offset += 1
        self.count += 1
        self.bitfield += c << self.bits
        self.bits += 8

    def snoopbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        return self.bitfield & BIT_MASKS[n]

    def readbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        r = self.bitfield & BIT_MASKS[n]
        self.bits -= n
        self.bitfield >>= n
        return r
...
class RBitfield(BitfieldBase):
    ...
    def snoopbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        return (self.bitfield >> (self.bits - n)) & BIT_MASKS[n]

    def readbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        r = (self.bitfield >> (self.bits - n)) & BIT_MASKS[n]
        self.bits -= n
        self.bitfield &= ~(BIT_MASKS[n] << self.bits)
        return r
...
class HuffmanTable(object):
    ...
    def populate_huffman_symbols(self):
        ...
        for x in self.table:
            ...
            forward, reverse = lookup_by_bits.setdefault(bits, ({}, {}))
            forward[x.symbol] = x.code
            reverse[x.reverse_symbol] = x.code
        self.lookup_by_bits = [
            (bits, forward, reverse)
            for bits, (forward, reverse) in sorted(lookup_by_bits.items())
        ]
    ...
    def find_next_symbol(self, field, reversed=True):
        lookup_index = 2 if reversed else 1
        for entry in self.lookup_by_bits:
            bits = entry[0]
            code = entry[lookup_index].get(field.snoopbits(bits))
            if code is not None:
                field.readbits(bits)
                return code
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = (
        "Pyflate with buffered input and indexed canonical-Huffman lookup"
    )
    runner.metadata['optimization'] = (
        "bulk buffered input, cached bit masks, grouped symbol lookup"
    )
